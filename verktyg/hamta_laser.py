...
# -*- coding: utf-8 -*-
"""
hamta_laser.py – laddar ner Laserdata Nedladdning, skog (LAZ/COPC)
från Lantmäteriets STAC-API för ett valt område.

Användning (Aspudden är förifyllt som standard):
  pip install requests
  python hamta_laser.py --user DITT_SYSTEMKONTO --password DITT_LOSENORD

Annat område (bbox i WGS84: lon_min lat_min lon_max lat_max):
  python hamta_laser.py --user X --password Y --bbox 17.98 59.29 18.02 59.32

Om API-adressen skiljer sig från standarden, ange den:
  python hamta_laser.py --user X --password Y --api https://api.lantmateriet.se/stac-hojd/v1
...
def main():
    ...
    ap.add_argument('--api', default='https://api.lantmateriet.se/stac-hojd/v1',
    ...
    ap.add_argument('--out', default='laz', help='mapp för nedladdade filer')
    a = ap.parse_args()
    ...
    lasercolls = [c for c in colls if 'laser' in c.lower() or 'skog' in c.lower()] or colls
